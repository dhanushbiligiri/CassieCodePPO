        ...
        try:
            ...
            while render_state:
                ...
                if (not env.vis.ispaused()):
                    ...
                    action = policy.forward(torch.Tensor(state), deterministic=True).detach().numpy() #action = policy.forward(torch.Tensor(state).to(self.device), deterministic=True).detach().cpu().numpy()
